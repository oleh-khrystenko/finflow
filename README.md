...
Комплексна B2B-платформа для автоматизації роботи бухгалтерських аутсорс-агенцій та мікробізнесу. Від генерації безкомісійних платіжних діплінків (НБУ) до повноцінної CRM-екосистеми для управління фінансами, пулом клієнтів (ФОПів) та звітністю.

Детальніше: [docs/vision/product.md](docs/vision/product.md).
...
- **Core** — авторизація, користувачі, платежі, shared UI, валідація, i18n. Стабільне ядро платформи.
- **Agency** — бізнес-модулі: платіжні лінки, дашборди для бухгалтерів, CRM-функціональність. Ізольований від core.
...
├── packages/
│   └── types/                # @finflow/types — Zod-схеми, типи, контракти
│       └── src/
...
| `pnpm --filter @finflow/types build`    | Build shared types          |

---

## Документація

- [Vision & Product](docs/vision/product.md) — концепція продукту та roadmap
