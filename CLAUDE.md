...
Схеми в `@finflow/types`, DTOs через `createZodDto()` з `nestjs-zod`.
...
- `RESEND_FROM_EMAIL` -> `'FinFlow <onboarding@resend.dev>'` (dev fallback)
...
# packages/types
pnpm --filter @finflow/types build                   # Compile to CJS in dist/
pnpm --filter @finflow/types dev                     # Watch mode
```
...
- API message responses: `{ data: { code: ResponseCode, message: string } }` -- `RESPONSE_CODE` з `@finflow/types`
...
- **Agency boundary**: Agency може імпортувати Core. Core НЕ може імпортувати Agency. Agency types -- окремий entry `@finflow/types/agency`.
...
`packages/types` МУСИТЬ бути зібраний до JS перед API/Web у Docker (enforced в docker-compose.dev.yml). **НІКОЛИ** не додавати `paths: { "@finflow/types": [...] }` до API tsconfig -- ламає структуру output. API резолвить через workspace symlink -> `dist/`. Web може мати `paths` (Next.js бандлер, points to source `../../packages/types/src/index.ts`).
